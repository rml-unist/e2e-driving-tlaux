...
def main():
    ...
    train_dataset_official = dataset_single_official.MORAIDataset(
        "./dataset", train=True,
        cache_file='dataset.pkl', ratio=0.8, config=config)
    ...
    config.detailed_loss_weights['loss_tl'] = 0.0
    for param in model.parameters():
      param.requires_grad = False
    for param in model.control_net.parameters():
      param.requires_grad = True
    model.load_state_dict(torch.load('./exp/w_aux_official/model_0050.pth'), strict=True)
