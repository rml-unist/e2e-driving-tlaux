...
def main():
  ...
  parser.add_argument('--logdir', type=str, default='./exp', help='Directory to log data and models to.')
  parser.add_argument('--root_dir', type=str, default="./dataset", help='Root directory of your training data.')
  parser.add_argument('--schedule_reduce_epoch_01', type=int, default=config.schedule_reduce_epoch_01, help='Epoch at which to reduce the lr by a factor of 10 the first time. Only used with --schedule 1.')
  parser.add_argument('--schedule_reduce_epoch_02', type=int, default=config.schedule_reduce_epoch_02, help='Epoch at which to reduce the lr by a factor of 10 the second time. Only used with --schedule 1.')
  parser.add_argument('--use_cosine_schedule', type=int, default=int(config.use_cosine_schedule), help='Whether to use a cyclic cosine learning rate schedule instead of the linear one.')
  parser.add_argument('--use_amp', type=int, default=int(config.use_amp), help='Whether to use automatic mixed precision with fp16 during training. Currently produces inf gradients.')
  parser.add_argument('--use_grad_clip', type=int, default=int(config.use_grad_clip), help='Whether to clip the gradients during training.')
  parser.add_argument('--gru_hidden_size', type=int, default=int(config.gru_hidden_size), help='Number of features used in the hidden size of the GRUs.')
  parser.add_argument('--learn_multi_task_weights', type=int, default=int(config.learn_multi_task_weights), help='Whether to learn the multi-task weights according to https://arxiv.org/abs/1705.07115.')
  parser.add_argument('--gru_input_size', type=int, default=int(config.gru_input_size), help='Number of channels in the InterFuser GRU input and Transformer decoder. Must be divisible by number of heads (8).')
  ...
  train_dataset_official = dataset_single_official.MORAIDataset("./dataset", train=True, cache_file='dataset.pkl', ratio=0.8, config=config)
  val_dataset_official = dataset_single_official.MORAIDataset("./dataset", train=False, cache_file='dataset.pkl', ratio=0.8, config=config)
  ...
  config.detailed_loss_weights['loss_ctrl'] = 0.0
  start_epoch = 0
  args.epochs = 51
  ...
  dataloader_train = DataLoader(train_dataset, batch_size=config.batch_size, generator=g_cuda, shuffle=False, worker_init_fn=seed_worker, num_workers=config.num_workers, drop_last=True, pin_memory=True)
  dataloader_val = DataLoader(val_dataset, batch_size=config.batch_size, generator=g_cuda, shuffle=False, worker_init_fn=seed_worker, num_workers=config.num_workers, drop_last=True, pin_memory=True)
  ...
  if config.use_cosine_schedule: # False
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=config.cosine_t0, T_mult=config.cosine_t_mult, verbose=False)
  else:
    milestones = [args.schedule_reduce_epoch_01, args.schedule_reduce_epoch_02]
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones, gamma=config.multi_step_lr_decay, verbose=True)
  ...
  for epoch in range(trainer.cur_epoch, args.epochs):
    ...
    logfile.write(f'Epoch: {epoch}, Total Loss: {train_loss_epoch}, Detailed Loss: {train_detailed_losses_epoch}, TL Acc: {tl_accuracy}\n')
    
    torch.cuda.empty_cache()

    val_loss_epoch, val_detailed_losses_epoch, tl_accuracy = trainer.validate() #! val
    logfile.write(f'Epoch: {epoch}, Total Loss: {val_loss_epoch}, Detailed Loss: {val_detailed_losses_epoch}, TL Acc: {tl_accuracy}\n\n')
    torch.cuda.empty_cache()
...
class Engine(object):
  ...
  def load_data_compute_loss(self, data, idx, validation=False):
    ...
    checkpoint = data['local_checkpoint'][:, :self.config.predict_checkpoint_len].to(self.device, dtype=torch.float32)
